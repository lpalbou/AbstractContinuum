...
/** Rewrite absolute hub URLs to the app-origin proxy path so images can
 *  embed under CSP img-src 'self' (the hub itself is another origin).
 *  Only attachment fetches rewrite — nothing else on the hub serves bytes
 *  the app should embed. */
 ...
 *    - image URLs (raster ext) that are app-origin (or hub attachment URLs,
 *      rewritten to the proxy) EMBED inline — his ruling scopes embedding
 ...
 *  is a same-origin proxy route other than a safe attachment. The whole
