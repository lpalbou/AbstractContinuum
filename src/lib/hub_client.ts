...
// mirror the hub API verbatim — the hub is the truth, this file only
// names what the Team page reads.
...
export class HubClient {
  ...
  async meta(): Promise<HubMeta> {
    return (await this._fetch("hub_meta", "/meta")) as HubMeta;
  }
  ...
  async fs_read(channel: string, path: string): Promise<HubFsFile> {
    // The path segments are pre-encoded so a slash-bearing path forwards
    // as-is (the proxy gate decodes for the allowlist, the raw path routes).
  ...
  /** Proxy URL to fetch an attachment's bytes (membership-gated hub-side;
   *  the seat key is attached server-side). Safe as an <img src> or a
  ...
  /** Grant/revise a delegation (operator-only admin surface; the proxy
   *  signs as the operator seat). Powers must be a non-empty subset of
  ...
  async rate_message(channel: string, message_id: string, value: 1 | -1, note = ""): Promise<any> {
    // Body typed against the GENERATED schema so hub_contract's CastRating
    // pin covers the code that ships, not a hand-written twin (adversary
    // P2-8): a hub-side field rename lands as a tsc error right here.
    const body: import("./hub_api_types").components["schemas"]["CastRating"] = { value, note };
    return await this._fetch(
      "hub_rate_message",
