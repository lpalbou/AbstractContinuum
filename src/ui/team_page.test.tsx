...
function make_gateway(overrides: Record<string, any> = {}) {
  return {
    backlog_advisor: vi.fn(async () => ({ reply: "the summary" })),
    ...overrides,
  } as any;
}

function render_page(gw = make_gateway()): void {
  render(<TeamPage gateway={gw} gateway_connected={true} />);
}
...
describe("acked-but-never-read badge (comms-audit ask 1, agora-0130)", () => {
  ...
  it("does NOT badge a swept-unread reply row inside a visible thread", async () => {
    stub_hub({
      messages: [
        { ...base, id: "01ROOT", seq: 4, sender: "laurent", status: "open", title: "root", body: "root body", read: true },
        { ...base, id: "01REPLY", seq: 5, sender: "agora", status: "reply", body: "reply body", read: false, reply_to: "01ROOT" },
      ],
      inbox: [],
    });
    render_page();
...
describe("top notification rails removed from TeamPage", () => {
  it("does not render the answers-waiting rail even when owed.to_consume is non-empty", async () => {
    stub_hub({
      owed: {
        to_consume: [
    ...
    expect(screen.queryByText(/answer to your question is waiting/i)).toBeNull();
    expect(screen.queryByText(/agora answered/i)).toBeNull();
  });

  it("does not render the top vigilance CTA; the count stays in the tab bar instead", async () => {
    stub_hub({
      messages: [{ channel: "commons", kind: "message", id: "01ASK", seq: 1, sender: "laurent", status: "open", title: "ask", body: "body", reply_to: null, created_at: 1, data: null }],
    });
    render_page();
    await screen.findByRole("button", { name: /^Needs vigilance/ });
...
describe("TeamPage moderation (operator dm 12: remove from a channel OR the hub)", () => {
  ...
  it("opens from cached last_seq without waiting for a slow /channels refresh", async () => {
    ...
    resolve_slow_channels?.(new Response(JSON.stringify([{ name: "commons", private: false, member: true, member_count: 2, last_seq: 400, last_at: now }]), { status: 200 }));
  });
});
...
describe("TeamPage threading + filters (2026-07-14 redesign)", () => {
  ...
  it("lets the operator fold a whole thread under its parent and reopen it", async () => {
    stub_hub({
      messages: [
        { ...base, id: "m1", seq: 1, sender: "core", status: "open", title: "root ask", body: "a", reply_to: null },
        { ...base, id: "m2", seq: 2, sender: "gateway", status: "reply", title: "the answer", body: "b", reply_to: "m1" },
    ...
    fireEvent.click(screen.getByText("fold thread"));
    await screen.findByText("show 1 reply");
    expect(screen.queryByText("the answer")).toBeNull();
    fireEvent.click(screen.getByText("show 1 reply"));
    await screen.findByText("the answer");
  });
  ...
  it("coalesces badge refreshes while one inbox/digest pass is still in flight", async () => {
    ...
    release_first_inbox?.(new Response(JSON.stringify([{ channel: "commons", seq: 8 }]), { status: 200 }));
    await waitFor(() => {
...
describe("TeamPage AI lanes (read-only contract)", () => {
  it("/assistant in the composer routes to the channel analyst and never posts to the room", async () => {
    ...
    const gw = make_gateway({ backlog_advisor: vi.fn(async () => ({ reply: "analysis: all quiet" })) });
    render(<TeamPage gateway={gw} gateway_connected={true} />);
    await screen.findByText(/as laurent/);
    ...
    expect(gw.backlog_advisor).toHaveBeenCalledTimes(1);
    const posts = fetch_mock.mock.calls.filter((c) => String(c[0]).includes("/messages") && String(c[1]?.method || "GET") === "POST");
    expect(posts).toHaveLength(0);
    // The question rode the channel transcript context.
    const q = String(gw.backlog_advisor.mock.calls[0][0].messages.at(-1).content || "");
    expect(q).toContain("what happened here?");
    expect(q).toContain("CHANNEL TRANSCRIPT");
  });

  it("thread AI summary calls the gateway advisor and renders the reply inline", async () => {
    stub_hub({
      messages: [
    ...
    const gw = make_gateway({ backlog_advisor: vi.fn(async () => ({ reply: "- decided: everything" })) });
    render(<TeamPage gateway={gw} gateway_connected={true} />);
  ...
  it("hides AI affordances when no gateway and no injected advisor", async () => {
    stub_hub({
      messages: [
        { channel: "commons", kind: "message", id: "m1", seq: 1, sender: "core", status: "open", title: "r", body: "x", reply_to: null, created_at: 1, data: null },
        { channel: "commons", kind: "message", id: "m2", seq: 2, sender: "uic", status: "reply", title: "a", body: "y", reply_to: "m1", created_at: 2, data: null },
