...
export function EmailInboxPanel(props: EmailInboxPanelProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    const stored = String(localStorage.getItem("abstractcontinuum_email_account") || "").trim();
  ...
  useEffect(() => {
    const chosen = String(account || "").trim();
    if (!chosen) return;
    localStorage.setItem("abstractcontinuum_email_account", chosen);
  }, [account]);
  ...
  async function refresh_list(): Promise<void> {
    ...
    try {
      ...
      set_messages(next);
      // Clear the selection when it is no longer IN the refreshed list
      // (account/mailbox/filter switch) — the old inverted condition never
      // fired, so the detail pane kept showing the previous account's
      // message (adversarial P2 2026-07-13).
      const current_uid = String(selected_uid || "").trim();
      if (current_uid && !next.some((m) => String(m?.uid || "").trim() === current_uid)) {
        set_selected_uid("");
        set_selected(null);
      }
    } catch (e: any) {
          ...
          <div className="inbox_list">
            {!messages.length ? (
              ...
              messages.map((m, idx) => {
                ...
                return (
                  <button key={uid || `idx:${idx}`} className={`inbox_item ${active ? "active" : ""}`} onClick={() => void open_message(uid)}>
