...
// - DMs are FIRST-CLASS since the operator's c2240 directive (2026-07-15):
//   the proxy admits dm:* reads/sends for HIS seat — his console, his own
//   ballots. (The v1 dm-exclusion protected AGENT surfaces; his call
//   supersedes it here. Do not "restore" the exclusion — that would be a
...
import { load_voice_override, voice_request_fields, resolve_default_voice_fields, speakable_text, VOICE_OVERRIDE_EVENT, type VoiceOverride } from "../lib/voice_settings";
...
/** Clamp heuristics: bodies longer than this get the CSS line-clamp +
 *  "read full" affordance. Rendering is ALWAYS the full markdown — CSS
 *  clamps visually, so a sliced code fence can never corrupt the render
 *  (adversary consensus find). */
const ROOT_CLAMP = 380;
const REPLY_CLAMP = 240;
...
const DRAFTS_KEY = "abstractcontinuum_team_drafts_v1";
...
/** Injected LLM lane — lets panel-hub shells call any backend without importing GatewayClient. */
export type TeamAdvisorFn = (question: string, history: TeamAiTurn[]) => Promise<string>;

export function TeamPage(props: {
  /** Voice/TTS only — optional when the shell has no gateway (AI may still work via advisor). */
  gateway?: GatewayClient;
  gateway_connected?: boolean;
  /** When set, powers thread summarize + channel assistant (read-only; never posts). */
  advisor?: TeamAdvisorFn;
  /** Gateway fallback only — ignored when advisor is injected. */
  ...
  const gateway_connected = Boolean(props.gateway_connected);
  const hub = useMemo(() => new HubClient(), []);

  const advisor_fn = useMemo((): TeamAdvisorFn | undefined => {
    if (props.advisor) return props.advisor;
    if (!gateway_connected || !gateway) return undefined;
    const agent = props.advisor_agent;
    return async (question: string, history: TeamAiTurn[]) => {
      const messages_payload = [
        ...history.map((t) => ({ role: t.role, content: t.content })),
        { role: "user" as const, content: question },
      ];
      const res = await gateway.backlog_advisor({ messages: messages_payload, agent: agent || null });
      return String(res?.reply || "").trim() || "(the model returned an empty reply)";
    };
  }, [props.advisor, gateway, gateway_connected, props.advisor_agent]);
  const ai_available = Boolean(advisor_fn);

  // Message voice (operator dm 130): each message is readable by the
  // gateway DEFAULT voice with STREAMING playback (speaks on the first
  // synthesized segment — no wait for the whole message); Settings can
  // override provider/model/voice (cloned included). TTS is a gateway run
  // effect, so it needs a run_id anchor — resolved ONCE against a stable
  // console voice session (the advisor's session_memory_run_id pattern).
  const [voice_run_id, set_voice_run_id] = useState<string>("");
  const [voice_override, set_voice_override] = useState<VoiceOverride>(() => load_voice_override());
  const [voice_error, set_voice_error] = useState<string>("");
  const voice_override_ref = useRef(voice_override);
  voice_override_ref.current = voice_override;
  // The operator's CONFIGURED default voice (dm 133): a bare TTS request
  // does NOT inherit it server-side (runtime stream lane doesn't merge
  // output.voice defaults), so the console sends it EXPLICITLY — else the
  // request falls through to abstractvoice's openai default. Fetched once
  // per connect; {} = nothing configured (let the gateway resolve).
  const voice_default_ref = useRef<{ provider?: string; model?: string; voice?: string }>({});
  useEffect(() => {
    if (!gateway_connected || !gateway) {
      set_voice_run_id("");
      return;
    }
    let alive = true;
    void (async () => {
      try {
        ...
        if (alive) set_voice_run_id(rid);
      } catch {
        if (alive) set_voice_run_id("");
      }
    })();
    // Resolve the operator's configured default voice (dm 133) so the
    // default speak path names his engine, not openai. Best-effort: an
    // unavailable endpoint or older client leaves {} and the request
    // stays bare. Feature-detect the method (test stubs / older client)
    // — a missing method throws synchronously, past any .catch.
    if (typeof (gateway as any).capability_defaults === "function") {
      void Promise.resolve()
        .then(() => gateway.capability_defaults())
        .then((payload) => {
          if (alive) voice_default_ref.current = resolve_default_voice_fields(payload);
        })
        .catch(() => {
          if (alive) voice_default_ref.current = {};
        });
    }
    return () => {
      alive = false;
  ...
  // Re-read the override when Settings changes it — the same-tab custom
  // event (P0-2: the Team page stays mounted behind Settings, so the
  // cross-tab `storage` event alone missed the primary flow) AND the
  // cross-tab storage event.
  useEffect(() => {
    const on_change = () => set_voice_override(load_voice_override());
    window.addEventListener(VOICE_OVERRIDE_EVENT, on_change);
    window.addEventListener("storage", on_change);
    return () => {
      window.removeEventListener(VOICE_OVERRIDE_EVENT, on_change);
      window.removeEventListener("storage", on_change);
    };
  }, []);
  const voice_stream_fn = useMemo(() => {
    const rid = String(voice_run_id || "").trim();
    if (!gateway_connected || !gateway || !rid) return undefined;
    return (text: string): AsyncIterable<ArrayBuffer> => {
      // Configured default UNDER the override (dm 133): the operator's
      // engine is named explicitly so a bare request never falls to the
      // openai default; the override then wins field-by-field.
      const t = gateway.voice_stream_transport(rid, { text, ...voice_default_ref.current, ...voice_request_fields(voice_override_ref.current) });
      return streamTtsJsonl({ path: t.path, body: t.body, headers: t.headers });
    };
    // voice_override read through the ref so a preference change never
    // re-mints the closure mid-playback (the hook keys stability on it).
  }, [gateway, gateway_connected, voice_run_id]);
  const message_voice = useGatewayVoice({ tts_stream: voice_stream_fn, on_error: set_voice_error });
  useEffect(() => {
    if (!voice_error) return;
    set_error(voice_error);
    const t = setTimeout(() => set_voice_error(""), 6000);
  ...
  /** Hub base URL (from /meta) — autolink uses it to rewrite pasted hub
   *  attachment URLs onto the app-origin proxy so they can embed under CSP. */
  ...
  /** Message ids expanded by explicit click (fires read_message once). */
  ...
  /** Threads whose reply trail is folded under the root row. */
  const [thread_folded, set_thread_folded] = useState<Record<string, boolean>>({});
  ...
  // Live updates: browser ⇄ proxy relay ⇄ hub /ws (seat key stays
  // server-side). Envelope frames trigger an INSTANT refresh of the
  // affected channel + a debounced badge refresh; the 5s poll keeps
  // running as the fallback lane (socket loss degrades, never breaks).
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    set_info(null);
    // Stop any in-flight read-aloud on channel switch (adversary P1-1):
    // the playing message's row unmounts, so its rail button — the only
    // pause control — would be unreachable while audio kept going.
    try {
      message_voice.stop_tts();
    } catch {
      /* hook not ready */
    }
  ...
  function toggle_expand(m: HubMessage): void {
    const next = !expanded[m.id];
    set_expanded((cur) => ({ ...cur, [m.id]: next }));
    // Explicit open = the hub's read_message record (critical unpin rides
    // it, c1696 rule 2). Once per message per mount; never moves cursors.
    if (next) fire_read(m);
  }

  /** Row click: expand/collapse + read tracking. Selection-aware — a
   *  click that ends a text selection never toggles (adversary find: the
   *  11px "read full" link was the only read affordance, and SHORT
   *  critical messages had none at all, staying pinned forever). */
  function row_click(m: HubMessage): void {
    const sel = window.getSelection();
    if (sel && !sel.isCollapsed) return;
    const body = String(m.body || "");
    const clamp = m.status === "reply" ? REPLY_CLAMP : ROOT_CLAMP;
    if (body.length > clamp) toggle_expand(m);
    else fire_read(m); // short message: no visual change, the read still records
  }
  ...
  async function react_on(target_id: string, dir: 1 | -1): Promise<void> {
    ...
    try {
      ...
      // (400 self, 409 retracted, 429 rate limit, route-absent proxy/hub)
  ...
  // null = listing unavailable (older proxy / hub error) — chips then mint
   ...
   *  to PREVIEW, not just download). Images show inline from the proxy URL;
   ...
   *  hub/proxy 404s the route and the failure surfaces honestly. */
  ...
  /** One advisor call. Both AI features ride the injected or gateway fallback fn. */
  ...
  async function run_group(arg: string): Promise<void> {
    ...
    if (!members.length) {
      set_error("/group: no @mentions found — usage: /group fix the voice outage @gateway @core");
      return;
    }
   ...
   *  the proxy and re-uploaded (idempotent by content hash). Failures NAME
  ...
  async function create_group_room(opts: { title: string; members: string[]; opening_body?: string }): Promise<void> {
    ...
    // the group's opening post by MIGRATION (fetch bytes via the proxy,
    ...
    try {
      ...
      // or older console proxy => 404/allowlist refusal => the original
      ...
      try {
        ...
        // #FALLBACK: pre-0.12.29 hub or pre-/groups console proxy — the
        // 4-call macro. Preflight the invite route BEFORE creating
        // anything (an old proxy would create the room, then fail every
  ...
  async function post(): Promise<void> {
    ...
    if (/^\/assistant(?=\s|$)/i.test(body)) {
      if (!ai_available) {
        set_error("AI advisor is not configured — connect a gateway or inject an advisor.");
        return;
      }
   ...
   *  allowlist, never SVG) render inline via the proxy URL with an onError
  ...
  function render_msg_body(m: HubMessage, opts: { is_root: boolean }): React.ReactElement {
    const is_open = Boolean(expanded[m.id]);
    const body = String(m.body || "");
    const clamp = opts.is_root ? ROOT_CLAMP : REPLY_CLAMP;
    const long = body.length > clamp;
    const clamped = long && !is_open;
    const asks = m.data?.asks || [];
    ...
    return (
      <div
        className={`team_row ${m.sender === seat ? "own" : ""} ${highlight ? "hit" : ""} ${replying ? "replying" : ""} ${unread ? "unread" : ""} ${m.retracted ? "retracted" : ""}`}
        key={m.id}
        id={`hubmsg-${m.id}`}
        onClick={() => row_click(m)}
        title={long ? undefined : m.critical ? "Click to record the read (critical unpins on it)" : undefined}
      >
        ...
        <div className="team_row_main">
          ...
          {/* Full markdown ALWAYS renders; CSS line-clamp handles the
              preview. A char-slice fed to the renderer shredded code
              fences (adversary consensus). Bare URLs autolink for display
              (operator dm 39/44): hub-internal images embed, the rest
              become clickable links — the stored message never changes. */}
          ...
          <div className={`team_row_body ${clamped ? (opts.is_root ? "clamp_root" : "clamp_reply") : ""}`}>
        ...
        {can_post ? (
          <div className="team_row_rail">
            ...
            {m.id && m.sender !== seat && m.sender !== "hub" && !m.retracted
              ...
              : null}
            {/* Speak (operator dm 130): read this message aloud through
                the gateway voice, streaming — playback starts on the first
                synthesized segment. Toggles play/pause on the same
                message; a second message stops the first. Rendered only
                when the gateway voice run is resolved. */}
            {message_voice.tts_supported && m.id
              ? (() => {
                  const playing = message_voice.tts_playback.key === m.id && message_voice.tts_playback.status === "playing";
                  const loading = message_voice.tts_playback.key === m.id && message_voice.tts_playback.status === "loading";
                  return (
                    <button
                      className={`btn btn_icon ${playing || loading ? "team_speak_on" : ""}`}
                      title={playing ? "Pause" : loading ? "Synthesizing…" : voice_override.enabled ? "Read aloud (your voice override)" : "Read aloud (gateway voice)"}
                      aria-label={playing ? "Pause reading" : "Read this message aloud"}
                      onClick={(e) => {
                        e.stopPropagation();
                        const speech = speakable_text(m.body || m.title || "");
                        // Empty speech (a code-only message) must SAY so,
                        // never a silent no-op on a rendered button (P2-3).
                        if (speech) void message_voice.toggle_tts(m.id, speech);
                        else set_error("Nothing readable to speak in this message (code/markup only).");
                      }}
                    >
                      <Icon name={playing ? "pause" : loading ? "refresh" : "speaker"} size={13} />
                    </button>
                  );
                })()
              : null}
  ...
  function render_thread(t: Thread): React.ReactElement {
    ...
    const folded = Boolean(thread_folded[t.root.id]);
    ...
    // find), so filters disable the collapse window.
    const shown = folded
      ? []
      : open_all || filter !== "all" || n <= COLLAPSED_REPLIES + 1
        ? t.replies
        : t.replies.slice(n - COLLAPSED_REPLIES);
    const hidden = folded ? n : n - shown.length;
    const s = summaries[t.root.id];
    return (
      <div className={`team_thread_group ${n ? "has_replies" : ""}`} key={t.root.id}>
        {t.orphan ? (
        ...
        {hidden > 0 || n > 0 || String(t.root.body || "").length > ROOT_CLAMP ? (
          <div className="team_trail_bar">
            {n > 0 ? (
              <button className="team_replies_more" onClick={() => set_thread_folded((cur) => ({ ...cur, [t.root.id]: !cur[t.root.id] }))}>
                {folded ? `show ${n} repl${n === 1 ? "y" : "ies"}` : "fold thread"}
              </button>
            ) : null}
            {!folded && hidden > 0 ? (
            ...
            {!folded && ai_available && (n > 0 || String(t.root.body || "").length > ROOT_CLAMP) ? (
              <button
        ...
        {!folded && n > 0 ? (
          <div className="team_replies">{shown.map((r) => render_msg(r, { is_root: false }))}</div>
        ) : null}
        {!folded && s?.text ? (
      ...
      <div className={`team_layout ${drawer ? "with_drawer" : ""} ${drawer === "leaderboard" ? "drawer_wide" : ""}`}>
        ...
        <div className="pane team_thread_pane">
          <div className="pane_header team_thread_header">
            ...
            <span className="pane_header_actions">
              {/* Always-reachable stop for a read-aloud (adversary P1-1):
                  the per-message speaker lives in the hover rail, which
                  unmounts when the message scrolls away — this pill is the
                  one control that is reachable while anything is playing. */}
              {message_voice.tts_playback.status !== "idle" ? (
                <button
                  className="btn team_speak_stop"
                  title="Stop reading aloud"
                  onClick={() => message_voice.stop_tts()}
                >
                  ◼ Stop reading
                </button>
              ) : null}
          ...
          >
                ...
                {group_preview.members.length ? (
                  ...
                  <span className="muted">
                    {group_preview.source === "kind"
                      ...
                      : "/group needs @mentions — e.g. /group fix the voice outage @gateway @core"}
                  </span>
                )}
              </div>
            ) : null}
            ...
            {!reply_to && !is_dm_channel && compose_kind === "group" ? (
              <input
                ...
                placeholder="Members — write their names: @entity @assistant @gateway"
                disabled={!can_post}
                ...
                placeholder={
                  can_post
                    ...
                    : health?.paused
                      ...
                      : "Read-only: the proxy has no operator seat key."
                }
                disabled={!can_post}
                onKeyDown={(e) => {
